...
void mergeRec(int* begin1, int* end1, int* begin2, int* end2, int* merged, size_t nrThreads)
{
    if(nrThreads <= 1 || end1-begin1<=0 || end2-begin2<=0) {
        merge(begin1, end1, begin2, end2, merged);
        return;
    }
    if(end1-begin1 < end2-begin2) {
        mergeRec(begin2, end2, begin1, end1, merged, nrThreads);
        return;
    }
    int* mid1 = begin1 + (end1-begin1)/2;
    int* mid2 = lower_bound(begin2, end2, *mid1);
    size_t sz1 = (mid1-begin1) + (mid2-begin2);
    size_t th = nrThreads/2;
    future<void> f1 = async(launch::async, [=]()->void{mergeRec(begin1, mid1, begin2, mid2, merged, th);});
    future<void> f2 = async(launch::async, [=]()->void{mergeRec(mid1, end1, mid2, end2, merged+sz1, nrThreads - th);});
    f1.wait();
    f2.wait();
}
...
void mergeSortRec(size_t n, int* in, int* buf, bool& isResultInInput, size_t nrThreads)
{
    ...
    if(r1) {
        mergeRec(in, in+k, in+k, in+n, buf, nrThreads);
        isResultInInput = false;
    } else {
        mergeRec(buf, buf+k, buf+k, buf+n, in, nrThreads);
        isResultInInput = true;
    }
}
