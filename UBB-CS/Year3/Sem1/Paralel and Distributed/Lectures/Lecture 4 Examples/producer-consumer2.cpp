...
public:
    void enqueue(T const& val) {
        ...
        //if (m_queue.size() >= ms_maxQueueSize)
        //{
        //    printf("Will wait\n");
        //}
        m_cv2.wait(lck, [this]() {return m_queue.size() < ms_maxQueueSize; });
        m_queue.push(val);
        m_cv.notify_one();
    }
...
private:
    ...
    static const int ms_maxQueueSize = 10;
};
...
int main(int argc, char** argv)
{
    int nrAdders;
    long long nrNumbers;
    if (argc != 3 || 1 != sscanf(argv[1], "%d", &nrAdders)
        || 1 != sscanf(argv[2], "%lld", &nrNumbers)) {
