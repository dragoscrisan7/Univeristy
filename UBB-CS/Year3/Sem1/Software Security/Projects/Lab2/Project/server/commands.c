    ...
    )
{
    ...
    tempPass[0] = '\0';
    if (strlen(Password) < DEFAULT_MAX_PASS_LEN) {
        strncpy(tempPass, Password, DEFAULT_MAX_PASS_LEN - 1);
        tempPass[DEFAULT_MAX_PASS_LEN - 1] = '\0';
    }
    else {
        // Handle the case where the input password is too long.
        return FALSE; // You might want to return an error code or handle it differently.
    }
