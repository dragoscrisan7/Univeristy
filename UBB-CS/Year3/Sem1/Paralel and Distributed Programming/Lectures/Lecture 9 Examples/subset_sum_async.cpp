...
public:
    ...
    {
        if (fullSum % 2 != 0) return; // odd total sum - cannot be split evenly
    ...
    {
        if (found != 0) return; // if another thread has already found a solution, we stop the search.
        if (pos >= values.size()) {
        ...
        if (nrThreads > 1) {
            ...
            subset2[pos] = false;
            std::future<void> other = std::async(std::launch::async, [this, pos, &subset2, targetSum, nrThreads]() {search(subset2, pos + 1, targetSum, nrThreads / 2); });
            subset[pos] = true;
