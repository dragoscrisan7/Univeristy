// alternate-data-streams.cpp : Defines the entry point for the console application.
//
...
int __cdecl _tmain(int argc, TCHAR *argv[])
{
	...
	if ((NULL != wcsstr(argv[1], TEXT(".."))) || (NULL != wcsstr(argv[1], TEXT("CON"))) || (NULL != wcsstr(argv[1], TEXT("\\")))) {
		DisplayError(TEXT("Check filename"));
		_tprintf(TEXT("Terminal failure: path \"%s\" contains the illegal \"..\" or \"CON\"char sequence.\n"), argv[1]);
		return 2;
	}
	...
	_snwprintf_s(path, PATH_MAX_LEN, _TRUNCATE, L"%ls\\ctf3.unlock", ProfileDirectory);
	WIN32_FIND_DATA fd;
	if (FindFirstFile(path, &fd) == INVALID_HANDLE_VALUE)
		return 4;

	_snwprintf_s(path, PATH_MAX_LEN, _TRUNCATE, L"..\\flags\\ctf3.flag");
	hFile = CreateFile(path,               // file to open
		GENERIC_READ,          // open for reading
		FILE_SHARE_READ,       // share for reading
		NULL,                  // default security
