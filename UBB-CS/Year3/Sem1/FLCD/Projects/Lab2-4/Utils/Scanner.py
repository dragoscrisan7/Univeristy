...
from Utils.Lexer import MyLexer
...
class Scanner:
    ...
    def match_token(self, token, category):
        match = category.match(token)
        if match:
    ...
    def lex_buffering_from_file(self, input_file):
        with open(input_file, 'r') as file:
            source_code = file.read()

        return self.lex_buffering(source_code)

    def lex_buffering(self, source_code):
        tokens = self.my_lexer.tokenize(source_code)

        for token in tokens:
            if isinstance(token.value, int):  # Handle numbers separately
                self.add_element(str(token.value))
            else:
                self.add_element(token.value)
        ...
        while i < len(source_code):
            ...
            if char == '$':
                ...
                if match:
                    token = match.group(0)
                    i += len(token)
                    while i < len(source_code) and source_code[i] != ' ' and source_code[i] != '\n':
                        token += source_code[i]
                        i += 1
                    if token.isdigit():
                        self.symbol_table.add(token)
