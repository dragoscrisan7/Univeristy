...
std::string generate_name(const std::string& other_name, const std::string& file, const std::string& type) {
    std::string final_name = other_name + type;

    if (file.size() >= 12) {
        std::string to_return = final_name + file.substr(12);
        return to_return;
    } else {
        // Handle the case where file is less than 12 characters
        return final_name + file;
    }
}
