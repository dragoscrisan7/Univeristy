...
void UserService::repoType(int fileType) {
    if (fileType == 1) {
        ...
        std::string userFile = R"(C:\Users\Dragos\OneDrive\Facultate\GitHub\a8-9-dragoscrisan7\AdoptedList.csv)";
        auto * repo = new csvfile{ userVector, userFile};
        this->urepo = repo;
    }else if (fileType == 2) {
        std::vector<dog> userVector ;
        std::string userFile = R"(C:\Users\Dragos\OneDrive\Facultate\GitHub\a8-9-dragoscrisan7\AdoptedList.html)";
